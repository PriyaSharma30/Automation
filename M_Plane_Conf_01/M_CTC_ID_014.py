...
def session_login(host, port, user, password, rmt, pswrd, slots):
    try:
        with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password, allow_agent=False, look_for_keys=False, timeout=60) as m:
            ...
            STARTUP.STORE_DATA('*'*100, OUTPUT_LIST=OUTPUT_LIST)
            STARTUP.STORE_DATA(
                '\n> get --filter-xpath /o-ran-software-management:software-inventory', OUTPUT_LIST=OUTPUT_LIST)
            STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
    ...
    <software-inventory xmlns="urn:o-ran:software-management:1.0">
            ...
            STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
            STARTUP.STORE_DATA = OUTPUT_LIST(
                f"{'SR_NO' : <20}{'Slot_Name' : <20}{'|' : ^10}{'Active': ^10}{'Running': ^10}", OUTPUT_LIST)
            STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
            k = 1
            for key, val in slots.items():
                STARTUP.STORE_DATA = OUTPUT_LIST(
                    f"{k : <20}{key : <20}{'=' : ^10}{val[0]: ^10}{val[1]: ^10}", OUTPUT_LIST)
                k += 1
            STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
            for key, val in slots.items():
                if val[0] == 'false' and val[1] == 'false':
                    ...
                    STARTUP.STORE_DATA(
                        '******* Replace with below xml ********', OUTPUT_LIST=OUTPUT_LIST)
                    STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
                    STARTUP.STORE_DATA(xml_data2, OUTPUT_LIST=OUTPUT_LIST)
                    STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
                    ...
                    STARTUP.STORE_DATA('*'*100, OUTPUT_LIST=OUTPUT_LIST)
                    STARTUP.STORE_DATA(
                        '\t\tStep 4 and 5 :  O-RU NETCONF Server sends <notification><install-event> with status COMPLETED to TER NETCONF Client', OUTPUT_LIST=OUTPUT_LIST)
                    STARTUP.STORE_DATA('*'*100, OUTPUT_LIST=OUTPUT_LIST)
                    while True:
                        n = m.take_notification()
                        if n == None:
                    ...
                    STARTUP.STORE_DATA('*'*100, OUTPUT_LIST=OUTPUT_LIST)
                    STARTUP.STORE_DATA(
                        '\t\t POST GET AFTER INSTALL SW', OUTPUT_LIST=OUTPUT_LIST)
                    STARTUP.STORE_DATA('*'*100, OUTPUT_LIST=OUTPUT_LIST)
                    STARTUP.STORE_DATA = OUTPUT_LIST(
                        '\n> get --filter-xpath /o-ran-software-management:software-inventory', OUTPUT_LIST)
                    STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
                    sw_inv = '''<filter xmlns="urn:ietf:params:xml:ns:netconf:base:1.0">
            <software-inventory xmlns="urn:o-ran:software-management:1.0">
            </software-inventory>
        ...
        STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
        STARTUP.STORE_DATA("*"*30+'FileNotFoundError' +
                           "*"*30, OUTPUT_LIST=OUTPUT_LIST)
        STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
        exc_type, exc_obj, exc_tb = sys.exc_info()
        STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
        ...
        STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
        STARTUP.STORE_DATA("*"*30+'XMLSyntaxError'+"*" *
                           30, OUTPUT_LIST=OUTPUT_LIST)
        STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
        exc_type, exc_obj, exc_tb = sys.exc_info()
        STARTUP.STORE_DATA('-'*100, OUTPUT_LIST=OUTPUT_LIST)
   ...
   # give the input in the format hostname, port, username, password
    ...
    try:
        ...
        * @credits Created based on Software Release for GIRU_revC-- v{val[2]}
        ...
        STARTUP.CREATE_LOGS('M_CTC_ID_014', OUTPUT_LIST)
