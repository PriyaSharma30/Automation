    ...
    def session_login(self):
        ...
        with manager.connect(host=self.host, port=self.port, username=self.user2, hostkey_verify=False, password=self.pswrd2, allow_agent = False , look_for_keys = False) as m:
            try:
                ...
                STARTUP.STORE_DATA('*'*100,OUTPUT_LIST=OUTPUT_LIST)
                STARTUP.STORE_DATA('\t\t Step 1 The TER NETCONF Client triggers <rpc><get> towards the O-RU NETCONF Server.',OUTPUT_LIST=OUTPUT_LIST)
                STARTUP.STORE_DATA('-'*100,OUTPUT_LIST=OUTPUT_LIST)
                STARTUP.STORE_DATA('> get',OUTPUT_LIST=OUTPUT_LIST)
                STARTUP.STORE_DATA('-'*100,OUTPUT_LIST=OUTPUT_LIST)
                STARTUP.STORE_DATA('\n\n',OUTPUT_LIST=OUTPUT_LIST)
                STARTUP.STORE_DATA('*'*100,OUTPUT_LIST=OUTPUT_LIST)
                STARTUP.STORE_DATA('\t\t Step 2 The O-RU NETCONF Server responds with <rpc-reply><data> where <data> contains all information elements that the O-RU NETCONF Server is able to expose',OUTPUT_LIST=OUTPUT_LIST)
                STARTUP.STORE_DATA('*'*100,OUTPUT_LIST=OUTPUT_LIST)
...
def test_MAIN_FUNC_010():
    ...
    try:
        ...
        * @credits Created based on Software Release for GIRU_revC-- v{val[2]}
        ...
        STARTUP.CREATE_LOGS('M_CTC_ID_010',OUTPUT_LIST)
