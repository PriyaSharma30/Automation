...
def session_login(host, port, user, password,s_n_i, g_t):
    with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password, allow_agent = False , look_for_keys = False) as m:
        ...
        for i in m.server_capabilities:
            STARTUP.STORE_DATA("\t{}".format(i),OUTPUT_LIST=OUTPUT_LIST)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    STARTUP.STORE_DATA('-'*100,OUTPUT_LIST=OUTPUT_LIST)
                    for i in range(1,31):
                        STARTUP.STORE_DATA(i,"th iteration",OUTPUT_LIST=OUTPUT_LIST)
        ...
        * @credits Created based on Software Release for GIRU_revC-- v{val[2]}
            ...
            if res:
                ...
                if type(res) == list:
                    ...
                    STARTUP.STORE_DATA('\n','*'*100,OUTPUT_LIST=OUTPUT_LIST)
                    STARTUP.STORE_DATA(f"{'FAIL-REASON' : <15}{'=' : ^10}{res : ^20}",OUTPUT_LIST=OUTPUT_LIST)
                    STARTUP.STORE_DATA('\n','*'*100,OUTPUT_LIST=OUTPUT_LIST)
                    # raise '\t\tFAIL-REASON\t\n{}'.format(res)
                STARTUP.STORE_DATA('\n','*'*100,OUTPUT_LIST=OUTPUT_LIST)
                STARTUP.STORE_DATA(f"{'STATUS' : <15}{'=' : ^10}{'FAIL' : ^20}",OUTPUT_LIST=OUTPUT_LIST)
                STARTUP.STORE_DATA('\n','*'*100,OUTPUT_LIST=OUTPUT_LIST)
                return res
                ...
                command = "cd {}; {} | grep supervision;".format(Config.details['SYSLOG_PATH'],Config.details['syslog_name'])
        ...
        STARTUP.CREATE_LOGS('M_CTC_ID_008',OUTPUT_LIST)
